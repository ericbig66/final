...
%>
<div class="card-deck col-10 mx-auto my-5">
<%
while(ac.next()){
...
<%out.println(introduce3);%>
						...
						<div type="button" class="btn btn-outline-success chinese-font mx-2" id="add_to_cart">加入購物車</div></a><!-- 後端注意:此開關為後端設計之範圍 --><button type='button' class='btn btn-outline-dark item_list_button mx-2 out_of_stock' id='out_of_stock' disabled><i class='fas fa-exclamation-circle'></i><div class='btn-text chinese-font'>&nbsp;缺貨</div></button><!-- 此按鈕請勿修改(庫存量為0時會自動使用) --><a href="dewish.jsp?proidn=<%=proid%>"><div type='button' class='btn btn-outline-info item_list_button mx-2 d-flex' id='add_to_wishList'><i class='fas fa-heart'></i><div class='btn-text chinese-font'>&nbsp;刪除</div></div></a><!-- 後端注意:此開關為後端設計之範圍 --></small></div></div>
